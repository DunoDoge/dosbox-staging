...
private:
	...
	void AutoMountDetectedDrive(char dir_letter);
...
static std::vector<std_fs::path> get_files_by_extension(
...
// settings
static std::string build_auto_mount_dir_command(
        const char dir_letter, const std_fs::path& drive_path,
...
static std::string build_auto_mount_cd_images_command(
...
// provided settings
static std::string build_auto_mount_floppy_images_command(
        const char dir_letter, const std::vector<std_fs::path>& image_paths,
        const std::optional<AutoMountSettings>& settings)
{
	auto command = CmdMount;
...
static std::string build_auto_mount_command(const char dir_letter,
                                            ...
                                            const std::optional<AutoMountSettings>& settings)
{
	...
	const auto image_paths = get_files_by_extension(drive_path, image_ext);
	...
	return build_auto_mount_dir_command(dir_letter, drive_path, settings);
}
...
void AutoExecModule::AutoMountDetectedDrive(const char dir_letter)
{
	...
	const auto settings  = parse_drive_conf(dir_letter, conf_path);

	// Install mount as an autoexec command
	AddLine(Placement::InitialAutogeneratedCommands,
	        build_auto_mount_command(dir_letter, drive_path, settings));
